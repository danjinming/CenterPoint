...
import math
import sys
sys.path.append('/home/daming/Work/HandDetection/CenterNetUlsB32/src/lib/models/networks')
import geffnet
import torch
...
def load_model(model,state_dict):
    ...
    # file = open('/home/daming/new_nn.txt', 'w')
    # for ss in new_keys:
    #     file.write(str(ss));
    #     file.write("\n")
    # file.close()
...
class IDAUp(nn.Module):
    def __init__(self, out_dim, channel):
        ...
        self.up = nn.Sequential(
                    ...
                    nn.Conv2d(out_dim, out_dim, kernel_size=1, stride=1, bias=False, groups=out_dim),
...
#         for head in self.heads:
#             if head == "hm":
#                 heat = self.__getattr__(head)(x).sigmoid_()
#                 ret[head] = nn.functional.max_pool2d(heat, (3, 3), stride=1, padding=1)
#             else:
#                 ret[head] = self.__getattr__(head)(x)
#         return [ret]
...
# num_layers  : [10 , 5]
def get_mobilenet_v2(num_layers, heads, head_conv=24):
